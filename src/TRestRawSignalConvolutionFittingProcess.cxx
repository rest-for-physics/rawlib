...
/// function
/// acts like a step function to select only the positive range of the AGET
/// function.
///
...
///
void TRestRawSignalConvolutionFittingProcess::LoadDefaultConfig() {
...
///
void TRestRawSignalConvolutionFittingProcess::LoadConfig(
...
///
TRestEvent *
TRestRawSignalConvolutionFittingProcess::ProcessEvent(TRestEvent *evInput) {
  // no need for verbose copy now
  fRawSignalEvent = (TRestRawSignalEvent *)evInput;

  debug << "TRestRawSignalConvolutionFittingProcess::ProcessEvent. Event ID : "
  ...
  for (int s = 0; s < fRawSignalEvent->GetNumberOfSignals(); s++) {
    ...
    TF1 *f = new TF1("Aget", "[0]*TMath::Exp(-3. * (x-[2])/[1]) * (x-[2])/[1] "
                             "* (x-[2])/[1] * (x-[2])/[1] * "
                             " sin((x-[2])/[1])/(1+TMath::Exp(-10000*(x-[2])))",
                     0, 511);
    ...
    fit_conv->SetParNames("Amplitude", "ShapingTime", "PeakPosition",
                          "VarianceGauss");
    ...
    fit_conv->SetParameters(singleSignal->GetData(MaxPeakBin), 25.,
                            MaxPeakBin - 25., 8.);
    fit_conv->FixParameter(0, singleSignal->GetData(MaxPeakBin));

    h->Fit(fit_conv, "LRNQ", "", MaxPeakBin - MinBinRange,
