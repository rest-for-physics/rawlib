...
TRestRawUSTCToSignalProcess::TRestRawUSTCToSignalProcess(char *cfgFileName) {
  Initialize();
}
...
void TRestRawUSTCToSignalProcess::InitProcess() {
  ...
  if ((!GetNextFrame(frame)) || (!ReadFrameData(frame))) {
    ...
    if ((!GetNextFrame(frame)) || (!ReadFrameData(frame))) {
      ferr << "TRestRawUSTCToSignalProcess: Failed to read the first data "
  ...
  if (fCurrentEvent != 0) {
    warning << "TRestRawUSTCToSignalProcess : first event is not with id 0 !"
            << endl;
    warning << "The first Id is " << fCurrentEvent
            << ". May be input file not the first file?" << endl;
  }
}
...
TRestEvent *TRestRawUSTCToSignalProcess::ProcessEvent(TRestEvent *evInput) {
  ...
  for (int i = 0; i < fEventBuffer[fCurrentBuffer].size(); i++) {
    ...
    if (frame->evId == fCurrentEvent && frame->eventTime == evtTime) {
      ...
      debug << "AsAdId, AgetId, chnId, max value: " << frame->boardId << ", "
            << frame->chipId << ", " << frame->channelId << ", "
            << sgnl.GetMaxValue() << endl;

    } else {
      warning << "TRestRawUSTCToSignalProcess : unmatched signal frame!"
              << endl;
      warning << "ID (supposed, received): " << fCurrentEvent << ", "
              << frame->evId << endl;
      warning << "Time (supposed, received) : " << evtTime << ", "
              << frame->eventTime << endl;
      warning << endl;
      fSignalEvent->SetOK(false);
      fCurrentEvent++;
      ClearBuffer();
...
void TRestRawUSTCToSignalProcess::EndProcess() {
  ...
  if (errorevents.size() > 0 && unknownerrors > 0) {
    warning << "There are also " << unknownerrors
            << " errors from unknown events! " << endl;
  } else if (unknownerrors > 0) {
    warning << "There are " << unknownerrors << " errors from unknown events! "
            << endl;
  }
...
bool TRestRawUSTCToSignalProcess::FillBuffer() {
  ...
  for (int i = 0; i < errorevents.size(); i++) {
    if (errorevents[i] == fCurrentEvent)
      return false;
  }
  return true;
}
...
#else
  if (fread(frame.data, DATA_SIZE, 1, fInputFiles[fCurrentFile]) != 1 ||
      feof(fInputFiles[fCurrentFile])) {
    fclose(fInputFiles[fCurrentFile]);
    fInputFiles[fCurrentFile] = NULL;
    return OpenNextFile(frame);
  ...
  while (1) {
    ...
    if (!(buffer[0] ^ 0xac) && !(buffer[1] ^ 0x0f)) {
      ...
      if (flag & 0x2) {
        ...
        n += HEADER_SIZE;
        warning << "successfully switched to next frame ( + " << n << " byte)"
                << endl;
        warning << endl;
        break;
      }
...
bool TRestRawUSTCToSignalProcess::ReadFrameData(USTCDataFrame &frame) {
  ...
  Long64_t tmp =
      (Long64_t)frame.data[5] * 0x10000 + (Long64_t)frame.data[6] * 0x100 +
      (Long64_t)frame
          .data[7]; // we omit the first byte in case the number is too large
  frame.eventTime = tmp * 0x100000000 + (Long64_t)frame.data[8] * 0x1000000 +
                    (Long64_t)frame.data[9] * 0x10000 +
                    (Long64_t)frame.data[10] * 0x100 + (Long64_t)frame.data[11];
  frame.channelId = frame.data[12] * 0x100 + frame.data[13];
  frame.evId = (frame.data[16] & 0x7F) * 0x1000000 + frame.data[17] * 0x10000 +
               frame.data[18] * 0x100 +
  ...
  frame.chipId = (frame.data[3] & 0xF0) / 16 - 10;
  frame.readoutType = frame.data[3] & 0x0F;
  Long64_t tmp = (Long64_t)frame.data[4] * 0x1000000 +
                 (Long64_t)frame.data[5] * 0x10000 +
  ...
  Long64_t tmp = (Long64_t)t_high * 0x100000000 + (Long64_t)t_mid * 0x10000 +
                 (Long64_t)t_low;
  frame.eventTime = tmp;
  ...
  for (int i = 0; i < 512; i++) {
    ...
    frame.dataPoint[i] =
        (int)((frame.data[pos] & 0x0F) * 0x100 + frame.data[pos + 1]);
  }
...
bool TRestRawUSTCToSignalProcess::AddBuffer(USTCDataFrame &frame) {
  ...
  if (frame.evId == fCurrentEvent) {
    ...
    if (pos >= fEventBuffer.size())
      pos -= fEventBuffer.size();
  ...
  if (frame.evId >= fCurrentEvent + fEventBuffer.size()) {
    ...
    warning << "this may due to the inconherence of event id. Increase the "
               "buffer number!"
            << endl;
    warning << "Current Event, Burrfering event : " << fCurrentEvent << ", "
            << frame.evId << endl;
    return false;
  }
  if (frame.evId < fCurrentEvent) {
