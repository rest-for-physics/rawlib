...
Int_t n = fRawSignalEvent->GetNumberOfSignals();

Double_t SigmaMean = 0;
Double_t Sigma[n];
Double_t RatioSigmaMaxPeakMean = 0;
Double_t RatioSigmaMaxPeak[n];

Double_t ChiSquareMean = 0;
Double_t ChiSquare[n];
Double_t VGmean = 0;
...
(singleSignal->GetRawData(i)-singleSignal->GetBaseLine())*100/singleSignal->GetData(MaxPeakBin));
                    ...
                    // Fit histogram with convolution
                    // fit_conv->SetParameters(32., MaxPeakBin - 25., 1.,
                    // singleSignal->GetData(MaxPeakBin)* 10);
                    ...
                    if (fAmplitudeInitialValue != 0) {
                        fit_conv->SetParameter(
                            3, singleSignal->GetData(MaxPeakBin) * fAmplitudeInitialValue);
                    } else {
                        fit_conv->SetParameter(3, singleSignal->GetData(MaxPeakBin) * 10);
                    }
                    ...
                    // fit_conv->FixParameter(3, singleSignal->GetData(MaxPeakBin) * 100);
                    // fit_conv->FixParameter(3, singleSignal->GetIntegralInRange(MaxPeakBin -
                    // MinBinRange, MaxPeakBin + MaxBinRange));
...
if (fAddAllPulses == true) {
    ...
    Double_t sigma = 0;
    for (int j = MaxPeakBin - MinBinRange; j < MaxPeakBin + MaxBinRange; j++) {
        sigma += (h->GetBinContent(j) - fit_conv->Eval(j)) * (h->GetBinContent(j) - fit_conv->Eval(j));
    }
    ...
    VGmean += fit_conv->GetParameter("VarianceGauss") / fit_conv->GetParError(2);
    VGvariance +=
        fit_conv->GetParameter("VarianceGauss") * fit_conv->GetParameter(2) / fit_conv->GetParError(2);
    VGweights += 1 / fit_conv->GetParError(2);
    ...
    SetObservableValue("FitRatioSigmaMaxPeakMean", TMath::Sqrt(sigma / (MinBinRange + MaxBinRange)) /
                                                       h->GetBinContent(MaxPeakBin + 1));
