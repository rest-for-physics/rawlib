...
///
TRestRawAFTERToSignalProcess::TRestRawAFTERToSignalProcess(char *cfgFileName) {
  Initialize();
}
...
void TRestRawAFTERToSignalProcess::InitProcess() {
  ...
  sscanf(runUid, "R%d.%02d.%02d-%02d:%02d:%02d", &year, &month, &day, &hour,
         &minute, &second);
  ...
  while (frameBits < payload) {
    ...
    if (first) // Timestamping (A. Tomas, 30th June 2011)
    {
      ...
      if (this->GetVerboseLevel() == REST_Debug) {
        ...
        cout << pHeader.ts_h << " -> " << ntohs(pHeader.ts_h) << " -> " << th
             << endl;
        cout << pHeader.ts_l << " -> " << ntohs(pHeader.ts_l) << " -> " << tl
             << endl;
    ...
    if (this->GetVerboseLevel() == REST_Debug) {
      ...
      cout << "RawDCC Head " << ntohs(pHeader.dcc) << " Version "
      ...
      cout << "FEM0Ind " << ntohs(pHeader.hdr) << " Type "
           << ((ntohs(pHeader.hdr) & 0xF000) >> 12);
      ...
      cout << "RawFEM " << ntohs(pHeader.args) << " M "
      ...
      cout << "RawEvType " << ntohs(pHeader.ecnt) << " EvTy "
    ...
    if (this->GetVerboseLevel() == REST_Debug) {
      cout << " channel " << channel << " asic " << asicN << " fec " << fecN
           << endl;
    }
    ...
    for (int i = 0; i < sampleCountRead; i++) {
      ...
      if (((data & 0xFE00) >> 9) == 8) {
        ...
        if (timeBin == 511)
          isData = false;
        if (this->GetVerboseLevel() == REST_Debug)
          cout << data << " Time bin " << timeBin << endl;
    ...
    if (this->GetVerboseLevel() == REST_Debug) {
      ...
      cout << "Readed "
           << sampleCountRead * sizeof(unsigned short) +
                  sizeof(DataPacketHeader) + sizeof(DataPacketEnd) +
                  sampleCountRead % 2 * sizeof(unsigned short)
           << " vs HeadSize " << ntohs(pHeader.size) << " Diff "
           << ntohs(pHeader.size) -
                  (sampleCountRead + sizeof(DataPacketHeader) +
                   sizeof(DataPacketEnd) + sampleCountRead % 2)
           << endl;
      cout << "Trailer_H " << ntohs(pEnd.crc1) << " Trailer_L "
           << ntohs(pEnd.crc2) << endl;
