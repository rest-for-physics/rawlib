...
void TRestRawSignalEvent::RemoveSignalWithId(Int_t sId) {
  ...
  if (index == -1) {
    cout << "Warning. Signal ID : " << sId
         << " does not exist. Signal will not be removed from signal event"
         << endl;
    return;
  }
...
Double_t TRestRawSignalEvent::GetThresholdIntegral() {
  Double_t sum = 0;
  for (int i = 0; i < GetNumberOfSignals(); i++)
    sum += fSignal[i].GetThresholdIntegral();
  return sum;
}
...
Double_t TRestRawSignalEvent::GetLowAverageWidth(Int_t nSignals,
...
void TRestRawSignalEvent::AddChargeToSignal(Int_t sgnlID, Int_t bin,
                                            Short_t value) {
...
TPad *TRestRawSignalEvent::DrawEvent(TString option) {
  ...
  char title[256];
  vector<TString> optList =
      Vector_cast<string, TString>(TRestTools::GetOptions((string)option));
  ...
  for (int j = 0; j < optList.size(); j++) {
    ...
    if (goodSigOpt != string::npos) {
      ...
      vector<TString> optList_2 =
          Vector_cast<string, TString>(Split((string)tmpStr, ","));
    ...
    if (BLOpt != string::npos) {
      ...
      vector<TString> optList_3 =
          Vector_cast<string, TString>(Split((string)tmpStr2, ","));
  ...
  if ((optList.size() == 0) || (isANumber((string)optList[0]) == false)) {
    ...
    if (ThresCheck == true && BLCheck == true) {
      ...
      debug << "  Base line range: (" << baseLineRangeInit << ","
            << baseLineRangeEnd << ")" << endl;

      for (int n = 0; n < nSignals; n++) {
        fSignal[n].CalculateBaseLine(baseLineRangeInit, baseLineRangeEnd);
        fSignal[n].InitializePointsOverThreshold(TVector2(pointTh, signalTh),
                                                 nOver);
        ...
        if (fSignal[n].GetPointsOverThreshold().size() >= 2 &&
            sRangeID == true) {
          if (fSignal[n].GetID() >= sRangeInit &&
              fSignal[n].GetID() <= sRangeEnd) {
            TGraph *gr = fSignal[n].GetGraph(n + 1);
            mg->Add(gr);
            sigPrinted++;
            if (printIDs == true) {
      ...
      if (ThresCheck == true && BLCheck == true) {
        ...
        for (int n = 0; n < nSignals; n++) {
          ...
          if (fSignal[n].GetPointsOverThreshold().size() >= 2 &&
            ...
            TGraph *gr = fSignal[n].GetGraph(n + 1);
            mg->Add(gr);
            if (fMinValue > fSignal[n].GetMinValue())
              fMinValue = fSignal[n].GetMinValue();
            if (fMaxValue < fSignal[n].GetMaxValue())
              fMaxValue = fSignal[n].GetMaxValue();
            sigPrinted++;
            if (printIDs == true) {
              cout << fSignal[n].GetID() << endl;
            }
        ...
        cout << "Number of good signals in range (" << firstSignal << ","
             << lastSignal << "): " << sigPrinted << endl;
      }
      // If no threshold and baseline options are given
      else {
