...
///
Double_t TRestRawSignal::GetData(Int_t n) {
  return (Double_t)fSignalData[n] - fBaseLine;
}
...
/// correction.
///
Double_t TRestRawSignal::GetRawData(Int_t n) {
  return (Double_t)fSignalData[n];
}
...
///
void TRestRawSignal::InitializePointsOverThreshold(TVector2 thrPar,
                                                   Int_t nPointsOver,
                                                   Int_t nPointsFlat) {
  if (fRange.X() < 0)
    fRange.SetX(0);
  ...
  for (int i = fRange.X(); i < fRange.Y(); i++) {
    ...
    if (this->GetData(i) > threshold) {
      ...
      if (pulse.size() >= nPointsOver) {
        ...
        // calculate stdev
        double mean =
            std::accumulate(pulse.begin(), pulse.end(), 0.0) / pulse.size();
        double sq_sum =
            std::inner_product(pulse.begin(), pulse.end(), pulse.begin(), 0.0);
...
void TRestRawSignal::CalculateThresholdIntegral() {
  ...
  for (unsigned int n = 0; n < fPointsOverThreshold.size(); n++) {
    if (fPointsOverThreshold[n] >= fRange.X() &&
        fPointsOverThreshold[n] < fRange.Y()) {
...
Double_t TRestRawSignal::GetSlopeIntegral() {
  ...
  Double_t sum = 0;
  for (unsigned int n = 0; n < fPointsOverThreshold.size(); n++) {
    if (n + 1 >= fPointsOverThreshold.size())
      return sum;

    sum += GetData(fPointsOverThreshold[n]);

    if (GetData(fPointsOverThreshold[n + 1]) -
            GetData(fPointsOverThreshold[n]) <
        0)
      break;
  }
  return sum;
...
///
void TRestRawSignal::GetDifferentialSignal(TRestRawSignal *diffSgnl,
                                           Int_t smearPoints) {
  if (smearPoints <= 0)
    smearPoints = 1;
  diffSgnl->Initialize();
  ...
  for (int i = smearPoints; i < this->GetNumberOfPoints() - smearPoints; i++) {
    Double_t value =
        0.5 * (this->GetData(i + smearPoints) - GetData(i - smearPoints)) /
        smearPoints;
...
///
void TRestRawSignal::GetWhiteNoiseSignal(TRestRawSignal *noiseSgnl,
                                         Double_t noiseLevel) {
  ...
  for (int i = 0; i < GetNumberOfPoints(); i++) {
    noiseSgnl->AddPoint(this->GetData(i) +
                        (Short_t)fRandom->Gaus(0, noiseLevel));
  }
  delete fRandom;
}
...
///
void TRestRawSignal::GetSignalSmoothed(TRestRawSignal *smthSignal,
                                       Int_t averagingPoints) {
  smthSignal->Initialize();

  averagingPoints =
      (averagingPoints / 2) * 2 + 1; // make it odd >= averagingPoints
  ...
  for (int i = averagingPoints / 2 + 1;
       i < GetNumberOfPoints() - averagingPoints / 2; i++) {
    sumAvg -= this->GetData(i - (averagingPoints / 2 + 1)) / averagingPoints;
    sumAvg += this->GetData(i + averagingPoints / 2) / averagingPoints;
    smthSignal->AddPoint((Short_t)sumAvg);
  }

  for (int i = GetNumberOfPoints() - averagingPoints / 2;
       i < GetNumberOfPoints(); i++)
...
void TRestRawSignal::CalculateBaseLineSigma(Int_t startBin, Int_t endBin) {
  if (endBin - startBin <= 0) {
    ...
    for (int i = startBin; i < endBin; i++)
      baseLineSigma +=
          (fBaseLine - fSignalData[i]) * (fBaseLine - fSignalData[i]);
...
void TRestRawSignal::AddOffset(Short_t offset) {
  if (fBaseLine != 0 || fBaseLineSigma != 0)
    fBaseLineSigma += (Double_t)offset;
  for (int i = 0; i < GetNumberOfPoints(); i++)
