...
///
TRestRawMemoryBufferToSignalProcess::TRestRawMemoryBufferToSignalProcess() {
  Initialize();
}
...
///
void TRestRawMemoryBufferToSignalProcess::LoadConfig(std::string cfgFilename,
                                                     std::string name) {
...
/// to process.
///
void TRestRawMemoryBufferToSignalProcess::BeginOfEventProcess() {
...
///
TRestEvent *
TRestRawMemoryBufferToSignalProcess::ProcessEvent(TRestEvent *evInput) {
  while (true) {
    ...
    if (dataReady == 2) {
      ...
      if (fOutputRawSignalEvent->GetNumberOfSignals() == 0)
        return NULL;
