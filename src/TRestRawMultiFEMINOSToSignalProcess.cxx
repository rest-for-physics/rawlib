...
#define PFX_START_OF_DFRAME                                                    \
  0x0800 // Start of Data Frame + 5 bit source + 4 bit Version
#define PFX_START_OF_MFRAME                                                    \
  0x0600 // Start of Monitoring Frame + 4 bit Version + 5 bit source
#define PFX_START_OF_CFRAME                                                    \
  0x0400 // Start of Configuration Frame + 4 bit Version + 5 bit source
//
...
#define PFX_EVPERIOD_HSTAT_BINS                                                \
  0x0007                        // Inter Event Time statistics and histogram
...
#define PFX_CARD_CHIP_CHAN_HIT_CNT                                             \
  0x8000 // Nb of Channel hit for given Card and Chip
...
#define PFX_START_OF_EVENT                                                     \
  0x00F0 // Start of Event + 1 bit free + Event Trigger Type
...
#define PUT_FVERSION_FEMID(w, fv, id)                                          \
...
TRestEvent *
TRestRawMultiFEMINOSToSignalProcess::ProcessEvent(TRestEvent *evInput) {
  ...
  while (1) {
    ...
    while (!endOfEvent) {
      ...
      while (!done) {
        ...
        if ((*sh & PFX_0_BIT_CONTENT_MASK) == PFX_START_OF_BUILT_EVENT) {
          ...
          nb_sh = fr_sz /
                  2;  // number of short words is half the event size in bytes
          ...
          nb_sh = fr_sz /
                  2;  // number of short words is half the frame size in bytes
      ...
      // Read binary frame
      if (!endOfEvent) {
        if (fread(&(cur_fr[fr_offset]), sizeof(unsigned short), nb_sh,
                  fInputBinFile) != nb_sh) {
    ...
    if (GetVerboseLevel() >= REST_Info) {
      ...
      if (GetVerboseLevel() >= REST_Debug) {
        for (Int_t n = 0; n < fSignalEvent->GetNumberOfSignals(); n++)
          cout << "Signal N : " << n
               << " daq id : " << fSignalEvent->GetSignal(n)->GetID() << endl;
        GetChar();
      }
    }
    ...
    if (fSignalEvent->GetNumberOfSignals() != 0) {
      ...
      warning << "blank event " << fSignalEvent->GetID() << "! skipping..."
              << endl;
    }
  }
  ...
  while (!done) {
    ...
    // Is it a prefix for 4-bit content?
      ...
      if (GetVerboseLevel() >= REST_Debug) {
        ...
        cout << "TimeStamp "
             << tStart + (2147483648 * r2 + 32768 * r1 + r0) * 2e-8 << endl;
      }
      ...
      if (fSignalEvent->GetID() == 0) {
        if (fLastEventId == 0) {
          ...
          fSignalEvent->SetTime(tStart +
                                (2147483648 * r2 + 32768 * r1 + r0) * 2e-8);
        } else {
          fSignalEvent->SetID(fLastEventId);
          fSignalEvent->SetTime(fLastTimeStamp);
        }
