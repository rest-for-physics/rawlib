...
Bool_t
TRestRawMultiCoBoAsAdToSignalProcess::InitializeStartTimeStampFromFilename(
    TString fName) {
  ...
  if (fName[fnOffset - 24] != '-' || fName[fnOffset - 21] != '-' ||
      ...
      fName[fnOffset - 12] != ':' || fName[fnOffset - 9] != '.' ||
      fName[fnOffset - 5] != '_') {
    cout << "Input binary file name unknown!" << endl;
    return kFALSE;
  }

  year = (int)(fName[fnOffset - 28] - 48) * 1000 +
         (int)(fName[fnOffset - 27] - 48) * 100 +
         (int)(fName[fnOffset - 26] - 48) * 10 +
         (int)(fName[fnOffset - 25] - 48) * 1;
  month = (int)(fName[fnOffset - 23] - 48) * 10 +
          (int)(fName[fnOffset - 22] - 48) * 1;
  day = (int)(fName[fnOffset - 20] - 48) * 10 +
        (int)(fName[fnOffset - 19] - 48) * 1;
  hour = (int)(fName[fnOffset - 17] - 48) * 10 +
         (int)(fName[fnOffset - 16] - 48) * 1;
  minute = (int)(fName[fnOffset - 14] - 48) * 10 +
           (int)(fName[fnOffset - 13] - 48) * 1;
  second = (int)(fName[fnOffset - 11] - 48) * 10 +
           (int)(fName[fnOffset - 10] - 48) * 1;
  millisecond = (int)(fName[fnOffset - 8] - 48) * 100 +
                (int)(fName[fnOffset - 7] - 48) * 10 +
                (int)(fName[fnOffset - 6] - 48) * 1;

  fStartTimeStamp.Set(year, month, day, hour, minute, second,
                      millisecond * 1000000, kTRUE,
...
TRestEvent *
TRestRawMultiCoBoAsAdToSignalProcess::ProcessEvent(TRestEvent *evInput) {
  ...
  if (GetVerboseLevel() >= REST_Debug) {
    cout << "TRestRawMultiCoBoAsAdToSignalProcess: Generating event with ID: "
         << fCurrentEvent << endl;
  }
      ...
      for (int m = 0; m < 272; m++) {
        if (data->chHit[m]) {
          ...
          if (GetVerboseLevel() >= REST_Extreme) {
            cout << "AgetId, chnId, first value, max value: " << m / 68 << ", "
                 << m % 68 << ", " << sgnl.GetData(0) << ", "
                 << sgnl.GetMaxValue() << endl;
          }
        }
      }
      data->evId = -1;
  ...
  if (GetVerboseLevel() >= REST_Debug) {
    cout << "TRestRawMultiCoBoAsAdToSignalProcess: event time is : " << tSt
         << endl;
    cout << "TRestRawMultiCoBoAsAdToSignalProcess: "
         << fSignalEvent->GetNumberOfSignals() << " signals added" << endl;
    cout << "------------------------------------" << endl;
  }
...
void TRestRawMultiCoBoAsAdToSignalProcess::EndProcess() {
  for (int i = 0; i < fileerrors.size(); i++) {
    if (fileerrors[i] > 0) {
      warning << "Found " << fileerrors[i] << " error frame headers in file "
              << i << endl;
...
bool TRestRawMultiCoBoAsAdToSignalProcess::fillbuffer() {
  ...
  if (fCurrentEvent == -1) {
    for (int i = 0; i < fInputFiles.size(); i++) {
      ...
      if (!ReadFrameHeader(fHeaderFrame[i])) {
        cout << "error when reading frame header in file " << i << " \""
             << fInputFileNames[i] << "\"" << endl;
        cout << "event id " << fCurrentEvent + 1 << ". The file will be closed"
             << endl;
        fHeaderFrame[i].Show();
        cout << endl;
        GetChar();
  ...
  for (int i = 1; i < fHeaderFrame.size(); i++) {
    if (fHeaderFrame[i].eventIdx < evt)
      evt = fHeaderFrame[i].eventIdx;
  }
  fCurrentEvent = evt;
  ...
  for (int i = 0; i < fHeaderFrame.size(); i++) {
    ...
    while (fHeaderFrame[i].eventIdx == fCurrentEvent) {
      ...
      if (fHeaderFrame[i].frameHeader[0] == 0x08 &&
          type == 1) // partial readout
      {
        ReadFrameDataP(fInputFiles[i], fHeaderFrame[i]);
      } else if (fHeaderFrame[i].frameHeader[0] == 0x08 &&
                 type == 2) // full readout
      {
        if (fread(frameDataF, 2048, 136, fInputFiles[i]) != 136 ||
            feof(fInputFiles[i])) {
          fclose(fInputFiles[i]);
          fInputFiles[i] = NULL;
      ...
      if (fread(fHeaderFrame[i].frameHeader, 256, 1, fInputFiles[i]) != 1 ||
        ...
        fclose(fInputFiles[i]);
        fInputFiles[i] = NULL;
        fHeaderFrame[i].eventIdx =
            (unsigned int)4294967295; // maximum of unsigned int
        break;
      }
      totalBytesReaded += 256;
      if (!ReadFrameHeader(fHeaderFrame[i])) {
        warning << "Event " << fCurrentEvent
                << " : error when reading next frame header" << endl;
        warning << "in file " << i << " \"" << fInputFileNames[i] << "\""
                << endl;
        ...
        for (int k = 0; k < 1088;
             k++) // fullreadoutsize(278528)/headersize(256)=1088
        {
          if (fread(fHeaderFrame[i].frameHeader, 256, 1, fInputFiles[i]) != 1 ||
              feof(fInputFiles[i])) {
            break;
          ...
          if (ReadFrameHeader(fHeaderFrame[i])) {
            fVerboseLevel = tmp;
            warning << "Successfully found next header (EventId : "
                    << fHeaderFrame[i].eventIdx << ")" << endl;
            if (fVerboseLevel > REST_Info)
              fHeaderFrame[i].Show();
            cout << endl;
            // GetChar();
...
bool TRestRawMultiCoBoAsAdToSignalProcess::ReadFrameHeader(
    CoBoHeaderFrame &HdrFrame) {
  UChar_t *Header = &(HdrFrame.frameHeader[0]);

  HdrFrame.frameSize = (unsigned int)Header[1] * 0x10000 +
                       (unsigned int)Header[2] * 0x100 +
                       (unsigned int)Header[3];
  HdrFrame.frameSize *= 256;

  HdrFrame.frameType =
      (unsigned int)Header[5] * 0x100 + (unsigned int)Header[6];
  HdrFrame.revision = (unsigned int)Header[7];
  HdrFrame.headerSize =
      (unsigned int)Header[8] * 0x100 + (unsigned int)Header[9];
  HdrFrame.itemSize =
      (unsigned int)Header[10] * 0x100 + (unsigned int)Header[11];
  HdrFrame.nItems = (unsigned int)Header[12] * 0x1000000 +
                    (unsigned int)Header[13] * 0x10000 +
                    (unsigned int)Header[14] * 0x100 + (unsigned int)Header[15];
  HdrFrame.eventTime = (Long64_t)Header[16] * 0x10000000000 +
                       (Long64_t)Header[17] * 0x100000000 +
                       (Long64_t)Header[18] * 0x1000000 +
                       (Long64_t)Header[19] * 0x10000 +
                       (Long64_t)Header[20] * 0x100 + (Long64_t)Header[21];
  HdrFrame.eventTime *= 10; // ns at 100MHz experiment clock
  HdrFrame.eventIdx = (unsigned int)Header[22] * 0x1000000 +
  ...
  HdrFrame.readOffset =
      (unsigned int)Header[28] * 0x100 + (unsigned int)Header[29];
  ...
  if (HdrFrame.nItems * HdrFrame.itemSize + 256 != HdrFrame.frameSize) {
    warning << "Event " << fCurrentEvent
            << " : item number and frame size unmatch!" << endl;
...
bool TRestRawMultiCoBoAsAdToSignalProcess::ReadFrameDataP(
    FILE *f, CoBoHeaderFrame &hdr) {
  unsigned int i;
  int j;
...
bool TRestRawMultiCoBoAsAdToSignalProcess::ReadFrameDataF(
    CoBoHeaderFrame &hdr) {
  int i;
  int j;
  unsigned int agetIdx, chanIdx, chanIdx0, chanIdx1, chanIdx2, chanIdx3,
      buckIdx, sample, chTmp;
  ...
  for (i = 0; i < 512; i++) {
    ...
    for (j = 0; j < 272; j++) {
      ...
      sample = ((unsigned int)(frameDataF[tmpP] & 0x0f) * 0x100 +
                frameDataF[tmpP + 1]);
...
Int_t TRestRawMultiCoBoAsAdToSignalProcess::GetLowestEventId() {
  ...
  for (unsigned int m = 1; m < fDataFrame.size(); m++) {
    if (evid == -1)
      evid = fDataFrame[m].evId;
    if (fDataFrame[m].evId != -1 && fDataFrame[m].evId < evid)
      evid = fDataFrame[m].evId;
  }
