...
Int_t REST_Raw_PlotVetoData(string fileName = "/home/konrad/VetoTest9Units/data/R01133_000_RawToSignal_Ar2Iso_BackgroundWith9Vetos_konrad_2.3.1.root", int starVal = 0, int endVal = 4000, int bins = 100){
			...
			for (unsigned int j=0; j<signalId.size(); j++){	
				...
				for (unsigned int n=0; n<run->GetEntries(); n++){
					...
					if (value > 0.1){
						...
						flag = 1; // flag, so that this histogram does not get a duplicate at the next observable iteration
					}
					peakTimeMap.clear();
				}
