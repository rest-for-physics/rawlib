...
public:
  ...
  void SetTailPoints(Int_t p) {
    for (int n = 0; n < GetNumberOfSignals(); n++)
      fSignal[n].SetTailPoints(p);
  }

  void SetBaseLineRange(TVector2 blRange) {
    SetBaseLineRange(blRange.X(), blRange.Y());
  }

  void SetBaseLineRange(Int_t from, Int_t to) {
    fBaseLineRange = TVector2(from, to);
    for (int n = 0; n < GetNumberOfSignals(); n++)
      fSignal[n].CalculateBaseLine(from, to);
  }
  ...
  Bool_t isBaseLineInitialized() {
    ...
    // signal
    for (int n = 0; n < GetNumberOfSignals(); n++)
      if (fSignal[n].isBaseLineInitialized())
        return true;
    return false;
  }
  ...
  Double_t GetLowAverageWidth(Int_t nSignals = 5,
